...
第一版默认启用第1至3类方法。第4类只能在已经实现并测试的分类型模型中使用，例如带资本约束的银行模型；第5类不作为通用后备方法启用。公司没有适用模型时输出 `unsupported`，不能因为历史DPS稳定就自动延续。
...
|---:|---|---:|---:|---:|---:|---|---:|
| 1 | 示例公用事业 | 80.00 | 25.00 | 0.90 | 3.60% | 未达目标区间 | 4.00%–5.00% |
| 2 | 示例银行 | 75.00 | 10.00 | 0.45 | 4.50% | 未达目标区间 | 5.00%–7.00% |
...
- 主表参考投资者分红观察表，只保留排名、公司、得分、股价、预期分红、预期股息率、当前位置和目标股息率区间；
...
- 中国平安：使用营运利润和中期股息趋势，不机械使用GAAP利润；
- 中国移动：已公告中期股息与全年派息率交叉验证；
- 招商银行或其他银行：30%左右派息率与资本约束；
- 利润骤降公司：不得照搬上一年度DPS；
...
7. 主表只保留排名、公司、综合得分、股价、预期DPS、预期股息率、位置和目标股息率区间。
8. 预期DPS不参与原选股分数。
...
中国电信因只有中期派息比例、缺少可作为全年预测输入的正式派息政策而保留 `data_gap`。中国移动可以形成A股口径预测DPS。2026-09-02按2026-08-31十年期国债收益率1.6883%重新计算后，目标股息率覆盖4.00%—8.00%；主表缩减为8列并保留正式得分。目标区间不依赖历史分位数，也没有用H股股息、行情源股息率或上一年DPS静默填充。
