...
- 前瞻事务保持独立读取已落盘的正式Top10；主命令默认在正式选股落盘后继续执行前瞻事务，前瞻失败不得破坏正式选股产物；
...
6. 从正式实施证据中识别下一次现金红利发放日，方案未实施时明确标记待公告。
7. 生成类似投资者分红观察表的简洁主表，同时提供逐家公司可追溯详情。
8. 任何进入主表的预期DPS和派息日期必须能从原始证据完整复算。
...
|---:|---|---:|---:|---:|---:|---|---|---:|---:|
| 1 | 示例公用事业 | 80.00 | 25.00 | 0.90 | 3.60% | 2026-09-10 | 未达目标区间 | 4.00%–5.00% | 18.00–22.50 |
| 2 | 示例银行 | 75.00 | 10.00 | 0.45 | 4.50% | 待实施公告 | 未达目标区间 | 5.50%–7.00% | 6.43–8.18 |
...
- 主表参考投资者分红观察表，只保留排名、公司、得分、股价、预期分红、预期股息率、下次派息日、当前位置、目标股息率区间和目标价格区间；
- 下次派息日只接受运行日后尚未除息的A股人民币常规分红正式日期；只有方案时显示“待实施公告”，无合格事件显示“—”，不得按历史节奏预测确切日期；
...
forward_12m_eligible_dps
next_dividend_date_status
next_dividend_event_id
next_dividend_record_date
next_dividend_ex_date
...
12. 主表只显示下次现金红利发放日；CSV和详情页保留对应登记日、除息日、事件ID和来源ID。
13. 已除息、特别股息、H股或非人民币事件不得成为下一次派息；只有方案但没有实施日期时不得伪造日期。
...
7. 主表只保留排名、公司、综合得分、股价、预期DPS、预期股息率、下次派息日、位置、目标股息率区间和目标价格区间。
8. 预期DPS不参与原选股分数。
...
- 独立运行稳定后，主runner默认执行前瞻事务，并提供 `--skip-forward-dividend` 显式退出；
- 正式选股事务必须先原子完成；
...
| 运行方式 | 前瞻事务独立读取正式Top10；主runner默认在正式选股落盘后继续执行，可用 `--skip-forward-dividend` 显式退出 |
...
2026-09-05将主runner改为默认双阶段入口：事务A先完整落盘，随后事务B读取正式Top10并输出前瞻主表；`--skip-forward-dividend`保留给明确只需正式选股产物的运行。独立前瞻命令继续用于事务B重跑和修复。
