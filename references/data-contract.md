...
The forward Top10 also carries the next formally evidenced A-share regular-dividend schedule: `next_dividend_date_status`, `next_dividend_event_id`, `next_dividend_record_date`, `next_dividend_ex_date`, `next_dividend_payment_date`, and `next_dividend_evidence_source_ids`. Select the earliest event whose ex-dividend date is after `run_date`; exclude already ex-dividend, special-dividend, H-share, and non-CNY events. The user-facing main table shows only the payment date. A current plan without an implementation date displays `待实施公告`; no qualifying evidence displays `—`. Never infer an exact date from historical cadence.
...
The replay manifest is the portable integrity root for a run. It hashes the formal Top10, every per-company `forecast-evidence.json`, every referenced original source document, the copied policy snapshot, and the forward CSV, and records the exact Git commit and runtime parameters. Offline replay must validate all hashes and reproduce every CSV field.
